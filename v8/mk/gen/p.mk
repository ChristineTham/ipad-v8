...
#     make -f $SRC/mk/gen/p.mk TOOLDIR=... install
#
...
CFLAGS = -O 
INCS   = -I$(SRC)/usr/src/cmd/p -I$(INCDIR)
COMPILE = $(CC) $(CFLAGS) $(INCS) -c
TOOLS  = $(CCPATH) $(CCOM) $(CPP) $(C2) $(AS)

OBJS = p.o pad.o spname.o

all: p

p: $(OBJS) $(LD) $(LIBC)
	$(CC) $(CFLAGS) -o p $(OBJS) $(LIBC)

p.o: $(SRC)/usr/src/cmd/p/p.c $(INCDIR)/sgtty.h $(INCDIR)/signal.h $(INCDIR)/stdio.h $(INCDIR)/sys/ioctl.h $(INCDIR)/sys/param.h $(INCDIR)/sys/types.h $(SRC)/usr/src/cmd/p/pad.h $(TOOLS)
	$(COMPILE) $(SRC)/usr/src/cmd/p/p.c

pad.o: $(SRC)/usr/src/cmd/p/pad.c $(INCDIR)/stdio.h $(SRC)/usr/src/cmd/p/pad.h $(TOOLS)
	$(COMPILE) $(SRC)/usr/src/cmd/p/pad.c

spname.o: $(SRC)/usr/src/cmd/p/spname.c $(INCDIR)/signal.h $(INCDIR)/sys/dir.h $(INCDIR)/sys/param.h $(INCDIR)/sys/types.h $(TOOLS)
	$(COMPILE) $(SRC)/usr/src/cmd/p/spname.c

install: p
	-mkdir $(DESTDIR)/usr/bin
	cp p $(DESTDIR)/usr/bin/p
