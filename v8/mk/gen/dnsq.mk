...
#     make -f $SRC/mk/gen/dnsq.mk TOOLDIR=... install
#
...
dnsq: $(OBJS) $(LD) $(LIBC) $(DESTDIR)/usr/lib/libin.a
	$(CC) $(CFLAGS) -o dnsq $(OBJS) $(DESTDIR)/usr/lib/libin.a $(LIBC)

dnsq.o: $(SRC)/usr/src/cmd/dnsq.c $(INCDIR)/signal.h $(INCDIR)/stdio.h $(INCDIR)/sys/inet/in.h $(INCDIR)/sys/inet/udp.h $(INCDIR)/sys/inet/udp_user.h $(INCDIR)/sys/param.h $(INCDIR)/sys/types.h $(TOOLS)
	$(COMPILE) $(SRC)/usr/src/cmd/dnsq.c

install: dnsq
	-mkdir $(DESTDIR)/usr
	-mkdir $(DESTDIR)/usr/inet
	-mkdir $(DESTDIR)/usr/inet/bin
	cp dnsq $(DESTDIR)/usr/inet/bin/dnsq
