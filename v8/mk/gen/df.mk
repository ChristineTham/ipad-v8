...
#     make -f $SRC/mk/gen/df.mk TOOLDIR=... install
#
...
CFLAGS = -O 
INCS   = -I$(SRC)/usr/src/cmd/df -I$(INCDIR)
...
df: $(OBJS) $(LD) $(LIBC)
	$(CC) $(CFLAGS) -o df $(OBJS) $(LIBC)

df.o: $(SRC)/usr/src/cmd/df/df.c $(INCDIR)/fstab.h $(INCDIR)/signal.h $(INCDIR)/stdio.h $(INCDIR)/sys/fblk.h $(INCDIR)/sys/filsys.h $(INCDIR)/sys/param.h $(INCDIR)/sys/stat.h $(INCDIR)/sys/types.h $(TOOLS)
	$(COMPILE) $(SRC)/usr/src/cmd/df/df.c

install: df
	-mkdir $(DESTDIR)/bin
	cp df $(DESTDIR)/bin/df
