...
#     make -f $SRC/mk/gen/time.mk TOOLDIR=... install
#
...
time: $(OBJS) $(LD) $(LIBC)
	$(CC) $(CFLAGS) -o time $(OBJS) $(LIBC)

time.o: $(SRC)/usr/src/cmd/time.c $(INCDIR)/signal.h $(INCDIR)/stdio.h $(INCDIR)/sys/param.h $(INCDIR)/sys/times.h $(INCDIR)/sys/types.h $(INCDIR)/sys/vtimes.h $(TOOLS)
	$(COMPILE) $(SRC)/usr/src/cmd/time.c

install: time
	-mkdir $(DESTDIR)/bin
	cp time $(DESTDIR)/bin/time
